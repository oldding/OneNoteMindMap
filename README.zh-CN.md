...
- **安全编辑** — 支持撤销/重做，新增节点自动进入编辑，误删和误操作可随时恢复
...
- **五种主题**：默认、紫色、简约、清新、温暖
- **三种节点形状**：圆角、矩形、胶囊
- **两种节点尺寸**：固定尺寸保持紧凑；适应内容按最大宽度自动换行并完整显示
- **四种连接线**：曲线、直线、折线、经典脑图
- **轻量表格**：OneNote 表格转换为紧凑的文本节点
- **导出**：支持导出为 PNG 或 SVG
- **视图控制**：支持适应窗口、恢复 100% 以及 Ctrl + 滚轮缩放
- **节点属性**：支持备注、链接、自定义颜色和 Emoji 图标
- **高效编辑**：支持节点搜索、方向键导航，以及带层级的复制粘贴
- **批量操作**：Ctrl + 单击多选，批量设置颜色和图标；Alt + ↑/↓ 调整同级顺序
...
1. 从 [Releases](https://github.com/oldding/OneNoteMindMap/releases) 下载安装程序
   - 64 位 OneNote 选择 `OneNoteMindMapSetup-1.5.2-x64.exe`
   - 32 位 OneNote 选择 `OneNoteMindMapSetup-1.5.2-x86.exe`
2. 运行安装程序（需要管理员权限）
3. 重启 OneNote
4. 在功能区可以看到新的 **脑图** 选项卡
